...
/*
	...
	There are N number of players in the game with each having some energy. The energy is used to kill the villains. 
...
public class Program08 {
	...
	public static void main(String[] args) {
		...
		for (int i = 0; i < testCases; i++) {
			...
			int[] villans = new int[numberOfPlayers];
			for (int j = 0; j < numberOfPlayers; j++) {
				villans[j] = sc.nextInt();
			}

			int[] heros = new int[numberOfPlayers];
			for (int j = 0; j < numberOfPlayers; j++) {
				heros[j] = sc.nextInt();
			}
